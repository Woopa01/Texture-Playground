...
//    }
    ...
    let label = ASTextNode().then {
        $0.attributedText = NSAttributedString(string: "Hello", attributes: [
    ...
    init() {
        ...
        node.layoutSpecBlock = { [weak self] (_,_) -> ASLayoutSpec in
            guard let strongSelf = self else { return ASLayoutSpec() }
            return ASInsetLayoutSpec(insets: .init(top: .infinity, left: .infinity, bottom: .infinity, right: .infinity), child: strongSelf.label)
        }
    }
//
    ...
    override func viewDidLoad() {
        label.addTarget(self, action: #selector(MainVC.goNext), forControlEvents: .touchDown)
    }
}
